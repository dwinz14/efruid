...
<div class="flex flex-col h-full bg-white text-slate-800" x-data="{
    ...
    <div
        class="relative overflow-hidden bg-gradient-to-br from-emerald-900 via-emerald-800 to-teal-800 text-white p-5 md:p-6 shrink-0 shadow-md">
        {{-- Background Geometric Glow --}}
        <div class="absolute -right-10 -top-10 w-40 h-40 bg-emerald-400/15 rounded-full blur-3xl pointer-events-none">
        </div>
        <div class="absolute -left-10 -bottom-10 w-36 h-36 bg-teal-300/10 rounded-full blur-2xl pointer-events-none">
        </div>
        ...
        <div class="relative z-10">
            <div class="flex items-start justify-between gap-3">
                <div class="flex items-center gap-3.5">
                    <div
                        class="w-11 h-11 rounded-2xl bg-white/15 backdrop-blur-md border border-white/25 flex items-center justify-center shadow-inner shrink-0 text-emerald-300">
                    ...
                    <div>
                        <div class="flex items-center gap-2">
                            <h2 class="text-sm font-extrabold tracking-wider uppercase text-white">Dokumen Sah & Asli
                            </h2>
                            <span
                                class="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-bold bg-emerald-400/20 text-emerald-200 border border-emerald-400/30">
                                RESMI
                            </span>
                        </div>
                ...
                <div class="hidden sm:block text-right shrink-0">
                    <span class="text-[10px] font-semibold text-emerald-200/80 block uppercase tracking-wider">BPR
                        Artha</span>
            ...
            <div
                class="mt-4 bg-white/10 border border-white/15 backdrop-blur-md rounded-xl py-2 px-3 flex items-center justify-between gap-2">
    ...
    <div class="flex-1 overflow-y-auto custom-scrollbar p-5 md:p-6 space-y-6">
        ...
        <div x-show="copiedText !== ''" x-transition.opacity.duration.200ms
            ...
            <svg class="w-4 h-4 text-emerald-400" fill="none" stroke="currentColor" stroke-width="2.5"
                viewBox="0 0 24 24">
        ...
        <div>
            ...
            <div class="bg-slate-50/80 border border-slate-200/80 rounded-2xl p-4 space-y-3.5 shadow-sm">
                ...
                <div class="flex items-start justify-between gap-2 pb-3 border-b border-slate-200/60">
                    <div>
                        <p class="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-0.5">Nomor Dokumen
                        </p>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                    ...
                    <div>
                        ...
                        <p class="font-semibold text-slate-800 mt-0.5">
                            {{ $permohonan->kantor?->nama ? ($permohonan->kantor->nama === 'PUSAT' ? 'KANTOR PUSAT' : 'CABANG ' . $permohonan->kantor->nama) : '—' }}
                        </p>
                            ...
                            @php
                                ...
                                $jenisBadge = match ($jenisVal) {
                                    ...
                                    default => [
                                        ...
                                        'label' => ucfirst($jenisVal),
                                    ],
                                };
                            @endphp
                            <span
                                class="inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-bold border {{ $jenisBadge['bg'] }}">
                                {{ $jenisBadge['label'] }}
                            </span>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        @if ($permohonan->access_level)
                            <span class="text-[10px] font-semibold text-slate-500">Level:
                                {{ $permohonan->access_level->value }}</span>
                        @endif
                    </div>
                </div>
                ...
                <div class="pt-2 border-t border-slate-200/60 flex items-center justify-between">
                    ...
                    <span
                        class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-100 text-emerald-800 text-[11px] font-extrabold tracking-wide border border-emerald-200">
                        <svg class="w-3.5 h-3.5 text-emerald-600" fill="none" stroke="currentColor" stroke-width="3"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                        </svg>
                        EXECUTED / SELESAI
        ...
        @if (count($stamps) > 0)
            <div>
                <div class="flex items-center justify-between mb-4">
                    <h3
                        class="text-[11px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                        <svg class="w-3.5 h-3.5 text-slate-400" fill="none" stroke="currentColor" stroke-width="2"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round"
                ...
                <div class="relative pl-3 border-l-2 border-emerald-200 space-y-4 ml-1.5">
                    @foreach ($stamps as $index => $stamp)
                        ...
                        <div class="relative pl-5 group">
                            {{-- Timeline Node Dot --}}
                            <div
                                class="absolute -left-[21px] top-2.5 w-3.5 h-3.5 rounded-full border-2 border-white {{ $colorMap['dot'] }} ring-2 ring-slate-100 shadow-sm transition-transform group-hover:scale-125">
                            </div>

                            {{-- Timeline Card --}}
                            <div
                                class="bg-white border border-slate-200/90 rounded-xl p-3.5 border-l-4 {{ $colorMap['border'] }} shadow-sm hover:shadow-md transition-all">
                                <div class="flex items-center justify-between gap-2 mb-1.5">
                                    <span
                                        class="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded {{ $colorMap['badge'] }}">
                                        {{ $aksiLabel }}
                                    </span>
                                    <span class="text-[10px] text-slate-400 font-mono">Tahap
                                        {{ $index + 1 }}</span>
                                </div>
                                ...
                                <div
                                    class="flex items-center flex-wrap gap-2 mt-2.5 pt-2 border-t border-slate-100 text-[10px]">
        ...
        <div
            class="bg-gradient-to-br from-slate-50 to-emerald-50/40 border border-emerald-100 rounded-2xl p-4 text-slate-600 space-y-2">
            ...
            <p class="text-[11px] text-slate-500 leading-relaxed">
                Dokumen ini dilindungi secara kriptografis menggunakan Personal Digital Seal & hash SHA-256. Setiap
                perubahan isi atau pemalsuan tanda tangan akan langsung menggugurkan keabsahan dokumen.
            </p>
        </div>
    ...
    <div class="bg-slate-50 border-t border-slate-200/80 p-4 text-center shrink-0">
        <p class="text-[10px] text-slate-400 leading-relaxed font-sans">
            Waktu Verifikasi: <span
                class="font-medium text-slate-600">{{ \Carbon\Carbon::now()->setTimezone('Asia/Jakarta')->isoFormat('D MMMM YYYY, HH:mm:ss') }}
                WIB</span><br>
            <span class="text-emerald-700 font-bold tracking-wider uppercase">eFRUID</span> &bull; PT BPR Artha
            Pamenang
        </p>
    </div>
</div>
