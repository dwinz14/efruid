...
@section('content')
    <div class="max-w-3xl mx-auto space-y-5">
        ...
        <div class="card card-body">
            ...
            {{-- Peringatan jika pemohon belum upload TTD --}}
            @if (!$permohonan->ttd_pemohon_path)
                <div class="alert-warning mt-4">
                    <svg class="w-4 h-4 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334
                                         -.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92z
                                         M11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002
                                         0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
                    </svg>
                    <span>Pemohon belum memiliki tanda tangan digital. TTD kolom pemohon akan kosong di dokumen.</span>
                </div>
            @endif
        </div>
        ...
        <div class="card card-body" x-data="approvalAction()">
            ...
            <h3 class="text-sm font-semibold text-slate-800 mb-4">Keputusan Anda</h3>

            {{-- Pilih aksi --}}
            <div class="grid grid-cols-2 gap-3 mb-5">
                <button type="button" @click="setAction('approve')" class="approval-btn-choice"
                    :class="action === 'approve' ? 'approval-btn-approve-active' : 'approval-btn-approve'">
                ...
                <button type="button" @click="setAction('reject')" class="approval-btn-choice"
                    :class="action === 'reject' ? 'approval-btn-reject-active' : 'approval-btn-reject'">
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            <div x-show="showApprove" x-transition>
                <div class="alert-info mb-4">
                    ...
                    <span>
                        Tanda tangan Anda akan disematkan ke dokumen.
                        @if (!auth()->user()->signature_path)
                            <strong class="text-amber-700">
                                Anda belum upload tanda tangan — kolom TTD Anda akan kosong.
                            </strong>
                        @endif
                    </span>
                </div>
                <form method="POST" action="{{ route('approval.atasan.approve', $permohonan) }}" @submit="loading = true">
                    @csrf
            ...
            <div x-show="showReject" x-transition>
                <form method="POST" action="{{ route('approval.atasan.reject', $permohonan) }}" @submit="loading = true">
                    @csrf
                    <div class="mb-4">
