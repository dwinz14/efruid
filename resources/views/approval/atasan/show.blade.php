    ...
    <div class="card card-body" x-data="approvalAction()">
        ...
        function approvalAction() {
            return {
                ...
                alasan: '',
                loading: false,
                setAction(val) { this.action = val; },
                get showReject() { return this.action === 'reject'; },
                get showApprove() { return this.action === 'approve'; },
            }
        }
        </script>
        ...
        <div x-show="showApprove" x-transition>
            ...
            <form
                method="POST"
                action="{{ route('approval.atasan.approve', $permohonan) }}"
                @submit="loading = true"
            >
                @csrf
        ...
        <div x-show="showReject" x-transition>
            <form
                method="POST"
                action="{{ route('approval.atasan.reject', $permohonan) }}"
                @submit="loading = true"
            >
                @csrf
                <div class="mb-4">
                    ...
                    <textarea
                        name="alasan_reject"
                        rows="3"
                        class="input @error('alasan_reject') input-error @enderror"
                ...
                <button
                    ...
                    :disabled="loading || alasan.length < 10"
                >
                    <svg x-show="loading" class="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10"
                                stroke="currentColor" stroke-width="4"/>
