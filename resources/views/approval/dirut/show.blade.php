...
@section('page-title', 'Persetujuan Direktur')
...
@section('content')
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 animate-fade-up">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <h2 class="text-lg sm:text-xl font-bold text-slate-800 tracking-tight mt-1">
                        Persetujuan Tingkat Direktur (FRUID)
                    </h2>
                </div>
            </div>
            ...
            <div class="flex items-center gap-2.5 flex-wrap flex-shrink-0">
                ...
                @if ($canApproveAsAtasan)
                    <span
                        class="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl text-xs font-bold bg-purple-50 text-purple-700 border border-purple-200 shadow-xs">
                        <span class="w-2 h-2 rounded-full bg-purple-500"></span>
                        Sebagai Atasan Langsung
                    </span>
                @elseif($canApproveAsDirut)
                    <span
                        class="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl text-xs font-bold bg-blue-50 text-blue-700 border border-blue-200 shadow-xs">
                ...
                {{-- Status Badge --}}
                <span
                    class="inline-flex items-center gap-1.5 px-3.5 py-1 rounded-xl text-xs font-bold border shadow-xs {{ $permohonan->status->badgeClass() }}">
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-5 sm:p-6 animate-fade-up"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                ...
                <div class="space-y-1">
                    ...
                    <div class="flex items-center gap-2.5">
                        <div
                            class="w-8 h-8 rounded-full bg-purple-100 text-purple-700 font-bold text-xs flex items-center justify-center flex-shrink-0">
                ...
                <div class="space-y-1">
                    ...
                    <div class="flex items-center gap-2 text-sm font-semibold text-slate-800 mt-1">
                        ...
                        <span class="truncate">{{ $permohonan->kantor?->label ?? ($permohonan->kantor?->nama ?? '—') }}</span>
                    </div>
                </div>
            ...
            {{-- Info Tambahan: Sudah disetujui Atasan sebelumnya --}}
            @if ($permohonan->atasan_id && $permohonan->status->value === 'PENDING_DIRUT')
                <div class="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between flex-wrap gap-2 text-xs">
                    <div class="flex items-center gap-2 text-slate-600">
                        <svg class="w-4 h-4 text-emerald-500 flex-shrink-0" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor" stroke-width="2.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                        </svg>
                        <span>Disetujui oleh Atasan: <strong
                                class="text-slate-800">{{ $permohonan->atasan?->name }}</strong></span>
                    </div>
                    <span class="text-slate-400">Menunggu persetujuan final Direktur</span>
                </div>
            @endif
        ...
        {{-- ── 4. Action Console: Keputusan Direktur ── --}}
            ...
            <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden animate-fade-up"
                ...
                <div class="px-5 sm:px-6 py-4 border-b border-slate-100 bg-slate-50/60 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="text-sm font-bold text-slate-800">Tindakan Persetujuan Direktur</h3>
                            <p class="text-xs text-slate-500 mt-0.5">
                                @if ($canApproveAsAtasan)
                                    Anda adalah atasan langsung. Persetujuan Anda akan meneruskan dokumen langsung ke IT.
                                @else
                ...
                <div class="p-5 sm:p-6 space-y-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                        ...
                        <button type="button" @click="setAction('reject')"
                            ...
                            <div class="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 transition-colors"
                                :class="action === 'reject' ? 'bg-red-600 text-white shadow-xs' :
                                    'bg-slate-100 text-slate-500'">
                                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                                    stroke-width="2.5">
                            ...
                            <div>
                                ...
                                <p class="text-xs mt-0.5"
                                    :class="action === 'reject' ? 'text-red-700' : 'text-slate-500'">
                    ...
                    <div x-show="showApprove" x-transition:enter="transition ease-out duration-250"
                        ...
                        @if (!auth()->user()->signature_path)
                            <div
                                ...
                                <div class="space-y-1">
                                    ...
                                    <p>Anda harus mengunggah tanda tangan digital terlebih dahulu di profil agar dapat
                                        melakukan approval.</p>
                        ...
                        @php
                            $approveActionRoute = $canApproveAsAtasan
                                ? route('approval.dirut.approveAsAtasan', $permohonan)
                                : route('approval.dirut.approve', $permohonan);
                        @endphp

                        <form method="POST" action="{{ $approveActionRoute }}" @submit="loading = true">
                            @csrf
                            <button type="submit"
                                ...
                                <span
                                    x-text="loading ? 'Memproses Persetujuan...' : 'Konfirmasi & Setujui Permohonan'"></span>
                            </button>
                        </form>
                    </div>
                    ...
                    <div x-show="showReject" x-transition:enter="transition ease-out duration-250"
                        ...
                        <div
                            class="p-4 rounded-xl bg-red-50 border border-red-200/80 flex items-start gap-3.5 text-red-800">
                            <div
                            ...
                            <div class="text-xs space-y-1">
                                <p class="font-bold text-red-900 text-sm">Konfirmasi Penolakan Direktur</p>
                                <p class="leading-relaxed">
                                    Permohonan ini akan ditolak secara resmi. Pemohon akan menerima notifikasi beserta
                                    catatan penolakan yang Anda masukkan.
                                </p>
                            </div>
                        </div>
                        ...
                        <form method="POST" action="{{ route('approval.dirut.reject', $permohonan) }}"
                            ...
                            <div>
                                ...
                                <textarea name="alasan_reject" rows="3" class="input w-full @error('alasan_reject') input-error @enderror"
                                    placeholder="Tuliskan secara jelas alasan penolakan permohonan ini..." x-model="alasan" required minlength="10"
                ...
                {{-- Footer Link --}}
                <div
                    class="px-5 sm:px-6 py-3.5 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between">
                    <a href="{{ route('approval.dirut.index') }}"
                    ...
                    <span class="text-xs text-slate-400">Pemeriksaan otentikasi dokumen tingkat Direksi</span>
                </div>
            </div>
        @endif
