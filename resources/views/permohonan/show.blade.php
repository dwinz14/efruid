            ...
            <div class="flex gap-2 mt-4 flex-wrap">
                ...
                @if ($permohonan->isRevisable())
                    <form method="POST" action="{{ route('permohonan.revise', $permohonan) }}"
                        onsubmit="return confirm('Kembalikan ke draft untuk direvisi?')">
                        @csrf
                        <button type="submit" class="btn-secondary btn-sm">Revisi</button>
                    </form>
                @endif
                    ...
                    <div class="space-y-4">
                        @foreach ($permohonan->approvalLogs as $log)
                            <div class="flex gap-3">
                                <div
                                    class="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center flex-shrink-0 mt-0.5">
                                ...
                                <div class="flex-1">
                                    <div class="flex items-center gap-2 flex-wrap">
                                        <p class="text-sm font-medium text-slate-800">{{ $log->user?->name ?? 'Sistem' }}
                                        </p>
                                        <span
                                            class="text-xs text-slate-400">{{ $log->created_at->locale('id')->isoFormat('D MMM Y HH:mm') }}</span>
                                    </div>
